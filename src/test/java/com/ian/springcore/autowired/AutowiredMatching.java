...
//    }
    ...
    /**
     * @Qualifier
     * Qualifier: 추가 구분자를 붙여주는 방식
     * - 주입 시 추가적인 방법을 제공하는 것이지 빈 이름 자체를 변경하는 것은 아님
     * - 주입 대상에 @Qualifier 애너테이션을 같이 붙임
     * 1. @Qualifier끼리 매칭
...
//    }
    ...
    /**
     * @Qualifier 사용 시 구분자를 문자로 붙이게 되는데 문자의 경우 컴파일 시 타입 체크가 안 됨
     * -> 애너테이션을 만들어서 사용하면 위와 같은 문제를 해결할 수 있음
     * - 기존 Qualifier 사용법과 일치하게 주입 대상에 생성한 애너테이션을 붙임
     */
    private final DiscountPolicy discountPolicy;

    @Autowired
    public AutowiredMatching(@MainDiscountPolicy DiscountPolicy discountPolicy) {
        this.discountPolicy = discountPolicy;
    }
}
